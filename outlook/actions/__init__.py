...
DEFAULT_SELECT: Final[list[str]] = ["from", "isRead", "receivedDateTime", "subject", "bodyPreview"]
...
@Client.decorator
async def folder_create(client: GraphServiceClient, name: str, parent_folder_id: str | None = None,) -> MailFolder | None:
    folders: MailFoldersRequestBuilder | ChildFoldersRequestBuilder

    if parent_folder_id:
        parent_folder: MailFolderItemRequestBuilder = client.me.mail_folders.by_mail_folder_id(parent_folder_id)
        folders = parent_folder.child_folders
    else:
        folders = client.me.mail_folders

    return await folders.post(MailFolder(display_name=name))


@Client.decorator
async def get_folders(client: GraphServiceClient) -> MailFolderCollectionResponse | None:
